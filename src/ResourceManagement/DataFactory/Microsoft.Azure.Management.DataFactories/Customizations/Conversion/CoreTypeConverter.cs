    ...
    {       
       #region Abstract methods
        ...
        protected virtual void ValidateTypeProperties(TExtensibleTypeProperties properties, Type type)
        {
            ...
            // TODO brgold: what about things hyak cannot validate?
            foreach (PropertyInfo property in
                    type.GetTypeInfo().GetProperties(ConversionCommon.DefaultBindingFlags)
                        .Where(p => !p.Name.Equals("ServiceExtraProperties", StringComparison.Ordinal)))
            {
                this.ValidateTypeProperty(property, properties);
            }
...
#else
            IEnumerable<PropertyInfo> props = property.PropertyType.GetTypeInfo().GetProperties(ConversionCommon.DefaultBindingFlags);
#endif
        ...
        {
            Type type = property.PropertyType;
            return type == typeof(IList<>) || (type.GetTypeInfo().IsGenericType && type.GetTypeInfo().GetGenericTypeDefinition() == typeof(IList<>))
                   || type.GetTypeInfo().GetInterfaces().Any(x => x.GetTypeInfo().IsGenericType && x.GetTypeInfo().GetGenericTypeDefinition() == typeof(IList<>));
        }
        ...
        private static bool ImplementsIDictionary(PropertyInfo property)
        {
            Type type = property.PropertyType;
            return type == typeof(IDictionary<,>)
                   || (type.GetTypeInfo().IsGenericType && type.GetTypeInfo().GetGenericTypeDefinition() == typeof(IDictionary<,>))
                   || type.GetTypeInfo().GetInterfaces()
                          .Any(x => x.GetTypeInfo().IsGenericType && x.GetTypeInfo().GetGenericTypeDefinition() == typeof(IDictionary<,>));
        }
        ...
        {
            Type type = propertyInfo.PropertyType;
            bool preCheck = type.GetTypeInfo().IsPrimitive || type.GetTypeInfo().IsValueType || type.GetTypeInfo().BaseType == typeof(object)
                            || propertyInfo.GetIndexParameters().Any();
        ...
        private static bool ShouldValidateMembers(Type type)
        {
            bool preCheck = type.GetTypeInfo().IsPrimitive || type.GetTypeInfo().IsValueType || type == typeof(string) || type == typeof(object);
            return !preCheck && IsNotNullable(type);
        }
        ...
        private static bool IsNotNullable(Type type)
        {
            ...
            if (underlyingType != null)
            {
                return !underlyingType.GetTypeInfo().IsValueType && !underlyingType.GetTypeInfo().IsPrimitive;
            }
