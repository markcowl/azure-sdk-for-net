...
#else
                    foreach (PropertyInfo pi in type.GetTypeInfo().GetProperties(BindingFlags.Public | BindingFlags.Instance))
                    {
                        object actualValue = type.GetTypeInfo().GetProperty(pi.Name).GetValue(actual, null);
                        object expectedValue = type.GetTypeInfo().GetProperty(pi.Name).GetValue(expected, null);
